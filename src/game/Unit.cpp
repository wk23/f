...
static Unit::AuraTypeSet GenerateAttakerProcEffectAuraTypes()
{
    ...
    auraTypes.insert(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK);
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    {
        const AreaTableEntry *area = GetAreaEntryByAreaID(pVictim->GetAreaId());
        if(area && area->flags & 0x800)//AREA_FLAG_SANCTUARY)       // sanctuary
            return;
    }
...
void Unit::SendAttackStateUpdate(CalcDamageInfo *damageInfo)
{
    ...
    data << (uint32)damageInfo->damageSchoolMask;
     ...
     if (auraSpellInfo->SpellIconID == 2006 && auraSpellInfo->SpellFamilyFlags==0x100000)
     {
                ...
                else if (auraSpellInfo->SpellFamilyFlags & UI64LIT(0x0000000000004000))
                {
                    ...
                    for(Unit::AuraList::const_iterator i = mAddFlatModifier.begin(); i != mAddFlatModifier.end(); ++i)
                    {
                        if ((*i)->GetModifier()->m_miscvalue == SPELLMOD_CHANCE_OF_SUCCESS && (*i)->GetSpellProto()->SpellIconID == 113)
                        {
                            ...
                            basepoints0 = value2 * GetMaxPower(POWER_MANA) / 100;// Drain Soul
         ...
         else if (auraSpellInfo->SpellFamilyFlags & 0x0000000000080000LL)
         {
            ...
            case SPELLFAMILY_SHAMAN:
            {
                ...
                if (auraSpellInfo->SpellFamilyFlags & UI64LIT(0x0000000000000400) && auraSpellInfo->SpellVisual==37)
                {
                    switch(auraSpellInfo->Id)
                    {
                        case 324:                           // Rank 1
...
{
    if(!spellProto || !pVictim || damagetype==DIRECT_DAMAGE ||  (spellProto->SpellIconID == 320))
        return pdamage;
...
//                    sLog.outDebug("     <end of list>");
        ...
        switch(auraModifier->m_auraname)
        {
            ...
            case SPELL_AURA_PRAYER_OF_MENDING:
            {
                ...
                HandleMendingAuraProc(triggeredByAura);//HandleMeandingAuraProc(triggeredByAura);
                break;
            }
