...
bool ChatHandler::HandleNpcChangeLevelCommand(const char* args)
{
    ...
    if(pCreature->isPet())
    {
        ...
        {
            pCreature->SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP, uint32((MaNGOS::XP::xp_to_level(lvl))/4));
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    if(target)
    {
        ...
        factionid = target->getFaction();
        factionrid = target->getFactionForRace(target->getRace());
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetMangosString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, security,factionid,factionrid,last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleWpModifyCommand(const char* args)
{
    ...
    if(show == "add" && target)
    {
        ...
        if (!wpCreature->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map,VISUAL_WAYPOINT,0))
        {
    ...
    if(show == "move" && target)
    {
        ...
        Map *map = chr->GetMap();
        {
            ...
            if( wpGuid != 0 )
            {
                ...
                if (!wpCreature2->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map, VISUAL_WAYPOINT, 0))
                {
                    PSendSysMessage(LANG_WAYPOINT_VP_NOTCREATED, VISUAL_WAYPOINT);
                    delete wpCreature2;
                    return false;
...
bool ChatHandler::HandleWpShowCommand(const char* args)
{
    ...
    if(show == "on")
    {
        ...
        do
        {
            ...
            Creature* wpCreature = new Creature;
            if (!wpCreature->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map, id, 0))
            {
    ...
    if(show == "first")
    {
        ...
        Creature* pCreature = new Creature;
        if (!pCreature->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT),map, id, 0))
        {
            PSendSysMessage(LANG_WAYPOINT_VP_NOTCREATED, id);
            delete pCreature;
            delete result;
